...
bothll <- function (beta, missing, x, weights, estimand, alpha, scratio) {
	...
	if (estimand %in% c("ATT", "MO")) {
		cb <- sum(((1 - missing) / (ps - 1) - missing * log(1 - ps) + missing * log(ps)) * weights)
	} else { # ATEcombined
...
bothV <- function (missing, ps, x, outcome, est, N, N1, weights, estimand, est.weights, alpha, scratio) {
	missing <- c(missing)
	if (estimand == "MO") {
		sumw0 <- sum((1 - missing) / (1 - ps) * weights)
		G0sc <- t(x) %*% (x * (c(1 - missing) * ps / (1 - ps) * (1 - scratio) - 
								c(alpha * missing - (alpha + 1) * ps) * ps^alpha * (1 - ps) * scratio) * weights) / N
		...
		G0sc <- t(x) %*% (-x * (c(1 - missing) * ps / (1 - ps) * (1 - scratio) + 
								c((alpha + 1) * ps - alpha * missing) * ps^alpha * (1 - ps) * scratio) * weights) / N
		g0sc <- -(c(((1 - missing) / (1 - ps)) - 1) * (1 - scratio) + 
		...
		G0sc1 <- t(x) %*% (x * (c(1 - missing) * ps1 / (1 - ps1) * (1 - scratio1) -
								c(alpha * missing - (alpha + 1) * ps1) * ps1^alpha * (1 - ps1) * scratio1) * weights) / N
		...
		G0sc2 <- t(x) %*% (x * (missing * (1 - ps2) / ps2 * (1 - scratio2) -
								c(alpha * (1 - missing) - (alpha + 1) * (1 - ps2)) * (1 - ps2)^alpha * ps2 * scratio2) * weights) / N
		g0sc2 <- (c((missing / ps2) - 1) * (1 - scratio2) - 
		...
		G0sc <- t(x) %*% (-x * (c(missing - ps)^2 / (ps * (1 - ps)) * (1 - scratio) -
							((alpha * (missing - ps) - ps) * ps^alpha * (1 - ps) +
									(alpha * (ps - missing) - (1 - ps)) * ps * (1 - ps)^alpha) * scratio) * weights) / N
		g0sc <- (c(missing / ps - (1 - missing) / (1 - ps)) * (1 - scratio) +
