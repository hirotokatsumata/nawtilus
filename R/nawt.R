...
nawt <- function (formula, outcome, estimand = "ATT", method = "score", 
									...
									boot = FALSE, B = 2000, clevel = 0.95, message = TRUE, data) {
	# Check
	...
	if (boot == TRUE) {
		...
		result <- nawt0(formula = formula, outcome = outcome, estimand = estimand, 
										method = method, alpha = alpha, twostep = twostep, 
										weights = weights, varcov = FALSE, data = data)
		...
		for (b in 1:B) {
			...
			result0 <- nawt0(formula = formula, outcome = outcome, estimand = estimand, 
											 method = method, alpha = alpha, twostep = twostep, 
											 weights = weights[bs.sample], varcov = FALSE, 
											 data = data[bs.sample, ])
			if (estimand != "ATE") {
		...
		result <- nawt0(formula = formula, outcome = outcome, estimand = estimand, 
										method = method, alpha = alpha, twostep = twostep, 
										weights = weights, varcov = TRUE, data = data)
		cilength <- 
